...
LABEL_DISPLAY_KEYS = {
    ...
    "Protein": ["name", "Entry", "Entry Name", "Protein names", "Gene Names (primary)", "id"],
}
NODE_COLORS = {
    "Pathway": "#1E88E5",
    "Enzyme": "#43A047",
...
def render_pyvis_graph(subgraph: dict[str, Any], height: int = 620) -> bool:
    ...
    net = Network(height=f"{height}px", width="100%", bgcolor="#FFFFFF", font_color="#1F2937")
...
def render_graph_page(driver) -> None:
    ...
    display_map = {int(row["node_id"]): f'{row["display"]} ({row["label"]}) | id={row["node_id"]}' for _, row in candidates_df.iterrows()}
